...
let rec remove a l = match l with
  | [] -> []
  | x :: q -> if x = a then q else x::(remove a q)

let rec print_clause c = match c with
  | [] -> ()
  | l :: c2 -> print_int l; print_char ' '; print_clause c2

let rec print_form f = match f with
  | [] -> ()
  | c :: f2 -> print_clause c; print_string "\n"; print_form f2
...
let rec count c i = match c with
  | [] -> 0, 0
  | l :: c2 -> let pos, neg = count c2 i in
...
let replace_neg l g f nb_vars = List.map (simpl_clause nb_vars) (List.map (replace_neg_clause l g) f)
...
let rec xelor f rho nb_vars = match f with
  | [] -> rho
  | c :: f2 -> (match c with
                  | [] -> raise Unsat
